...
local function OpenGarageMenu(data)
    KGCore.Functions.TriggerCallback('kg-garages:server:GetGarageVehicles', function(result)
        if result == nil then return KGCore.Functions.Notify(Lang:t('error.no_vehicles'), 'error', 5000) end
        ...
        for _, v in pairs(result) do
            ...
            local vname = nil
            pcall(function()
                vname = KGCore.Shared.Vehicles[v.vehicle].name
            end)
...
local function DepositVehicle(veh, data)
    local plate = KGCore.Functions.GetPlate(veh)
    KGCore.Functions.TriggerCallback('kg-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            ...
            TriggerServerEvent('kg-garages:server:updateVehicleStats', plate, totalFuel, engineDamage, bodyDamage)
            CheckPlayers(veh)
            if plate then TriggerServerEvent('kg-garages:server:UpdateOutsideVehicle', plate, nil) end
            KGCore.Functions.Notify(Lang:t('success.vehicle_parked'), 'primary', 4500)
        else
            KGCore.Functions.Notify(Lang:t('error.not_owned'), 'error', 3500)
        end
...
local function CreateBlipsZones()
    ...
    comboZone:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
                    ...
                    if IsControlJustReleased(0, 38) then
                        if GetVehiclePedIsUsing(PlayerPedId()) ~= 0 then
                            ...
                            if not IsVehicleAllowed(zone.data.category, currentVehicle) then
                                KGCore.Functions.Notify(Lang:t('error.not_correct_type'), 'error', 3500)
                                return
                            end
            ...
            exports['kg-core']:DrawText(displayText, 'left')
        else
            listenForKey = false
...
function GetSpawnPoint(garage)
    ...
    if not location then
        KGCore.Functions.Notify(Lang:t('error.vehicle_occupied'), 'error')
    end
    return location
end
...
RegisterNUICallback('takeOutVehicle', function(data, cb)
    TriggerEvent('kg-garages:client:takeOutGarage', data)
    cb('ok')
end)

RegisterNUICallback('trackVehicle', function(plate, cb)
    TriggerServerEvent('kg-garages:server:trackVehicle', plate)
    cb('ok')
end)
...
RegisterNUICallback('takeOutDepo', function(data, cb)
    ...
    if depotPrice ~= 0 then
        TriggerServerEvent('kg-garages:server:PayDepotPrice', data)
    else
        TriggerEvent('kg-garages:client:takeOutGarage', data)
    end
    cb('ok')
...
RegisterNetEvent('kg-garages:client:trackVehicle', function(coords)
...
RegisterNetEvent('kg-garages:client:takeOutGarage', function(data)
    KGCore.Functions.TriggerCallback('kg-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            local location = GetSpawnPoint(data.garage)
            if not location then return end
            KGCore.Functions.TriggerCallback('kg-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                while not NetworkDoesNetworkIdExist(netId) do Wait(10) end
                local veh = NetworkGetEntityFromNetworkId(netId)
                Citizen.Await(CheckPlate(veh, vehPlate))
                KGCore.Functions.SetVehicleProperties(veh, properties)
                exports[Config.FuelResource]:SetFuel(veh, data.stats.fuel)
                TriggerServerEvent('kg-garages:server:updateVehicleState', 0, vehPlate)
            ...
            KGCore.Functions.Notify(Lang:t('error.not_depot'), 'error', 5000)
        end
...
local function CreateHouseZone(index, garage, zoneType)
    ...
    houseComboZones:onPlayerInOut(function(isPointInside, _, zone)
        if isPointInside then
            ...
            exports['kg-core']:DrawText(Lang:t('info.house_garage'), 'left')
        else
...
RegisterNetEvent('kg-garages:client:setHouseGarage', function(house, hasKey) -- event sent periodically from housing
    ...
    if Config.Garages[formattedHouseName] then
        ...
        KGCore.Functions.TriggerCallback('kg-garages:server:getHouseGarage', function(garageInfo) -- create garage if not exist
            ...
            TriggerServerEvent('kg-garages:server:syncGarage', Config.Garages)
        end, house)
    end
end)

RegisterNetEvent('kg-garages:client:houseGarageConfig', function(houseGarages)
...
RegisterNetEvent('kg-garages:client:addHouseGarage', function(house, garageInfo) -- event from housing on garage creation
    local formattedHouseName = string.gsub(string.lower(house), ' ', '')
    Config.Garages[formattedHouseName] = {
        houseName = house,
...
RegisterNetEvent('KGCore:Client:OnGangUpdate', function(gang)
    PlayerGang = gang
end)

RegisterNetEvent('KGCore:Client:OnJobUpdate', function(job)
    PlayerJob = job
end)
