...
local function filterVehiclesByCategory(vehicles, category)
    ...
    for _, vehicle in pairs(vehicles) do
        local vehicleData = KGCore.Shared.Vehicles[vehicle.vehicle]
...
KGCore.Functions.CreateCallback('kg-garages:server:getHouseGarage', function(_, cb, house)
    local houseInfo = MySQL.single.await('SELECT * FROM houselocations WHERE name = ?', { house })
    cb(houseInfo)
end)

KGCore.Functions.CreateCallback('kg-garages:server:GetGarageVehicles', function(source, cb, garage, type, category)
...
local function GetVehicleTypeByModel(model)
    local vehicleData = KGCore.Shared.Vehicles[model]
...
KGCore.Functions.CreateCallback('kg-garages:server:spawnvehicle', function(source, cb, plate, vehicle, coords)
    local vehType = KGCore.Shared.Vehicles[vehicle] and KGCore.Shared.Vehicles[vehicle].type or GetVehicleTypeByModel(vehicle)
...
KGCore.Functions.CreateCallback('kg-garages:server:IsSpawnOk', function(_, cb, plate, type)
...
KGCore.Functions.CreateCallback('kg-garages:server:canDeposit', function(source, cb, plate, type, garage, state)
    local Player = KGCore.Functions.GetPlayer(source)
    ...
    if type == 'house' and not exports['kg-houses']:hasKey(Player.PlayerData.license, Player.PlayerData.citizenid, Config.Garages[garage].houseName) then
        cb(false)
        return
    end
    if state == 1 then
...
RegisterNetEvent('kg-garages:server:updateVehicleStats', function(plate, fuel, engine, body)
    local src = source
...
RegisterNetEvent('kg-garages:server:updateVehicleState', function(state, plate)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('kg-garages:server:UpdateOutsideVehicle', function(plate, vehicleNetID)
...
RegisterNetEvent('kg-garages:server:trackVehicle', function(plate)
    local src = source
    local vehicleData = OutsideVehicles[plate]
    if vehicleData and DoesEntityExist(vehicleData.entity) then
        TriggerClientEvent('kg-garages:client:trackVehicle', src, GetEntityCoords(vehicleData.entity))
        TriggerClientEvent('KGCore:Notify', src, Lang:t('success.vehicle_tracked'), 'success')
    else
        TriggerClientEvent('KGCore:Notify', src, Lang:t('error.vehicle_not_tracked'), 'error')
    end
end)

RegisterNetEvent('kg-garages:server:PayDepotPrice', function(data)
    local src = source
    local Player = KGCore.Functions.GetPlayer(src)
    ...
    MySQL.scalar('SELECT depotprice FROM player_vehicles WHERE plate = ?', { data.plate }, function(result)
        if result then
            ...
            if cashBalance >= depotPrice then
                ...
                TriggerClientEvent('kg-garages:client:takeOutGarage', src, data)
            elseif bankBalance >= depotPrice then
                Player.Functions.RemoveMoney('bank', depotPrice, 'paid-depot')
                TriggerClientEvent('kg-garages:client:takeOutGarage', src, data)
            else
                TriggerClientEvent('KGCore:Notify', src, Lang:t('error.not_enough'), 'error')
            end
        end
    end)
end)
...
RegisterNetEvent('kg-garages:server:syncGarage', function(updatedGarages)
    Config.Garages = updatedGarages
end)

--Call from kg-phone

KGCore.Functions.CreateCallback('kg-garages:server:GetPlayerVehicles', function(source, cb)
    local Player = KGCore.Functions.GetPlayer(source)
    local Vehicles = {}
    ...
    MySQL.rawExecute('SELECT * FROM player_vehicles WHERE citizenid = ?', { Player.PlayerData.citizenid }, function(result)
        if result[1] then
            for _, v in pairs(result) do
                local VehicleData = KGCore.Shared.Vehicles[v.vehicle]
